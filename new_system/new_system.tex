...
\section{処理速度の課題}
第3章で述べた提案手法は，リアルタイム性を向上するゼロショットセグメンテーションを実現する一方で，処理速度（FPS）が十分に得られないという課題がある．特に，SAM2は大規模なモデルであり，毎フレーム実行すると処理速度が低下し，リアルタイム性が損なわれる可能性がある．この問題を解決するために，本章では処理速度に着目した提案手法の処理について検討する．
...
\section{各処理の詳細}
処理速度に着目した提案手法の処理について述べる．\Fig{fig:system_ver2}に示すように，処理速度に着目した提案手法の処理は，(1) 物体検出およびセグメンテーション(2) 物体追跡，の 2 つの処理で構成される．以下にそれぞれの処理に関して述べる．
...
\section{提案手法の性能評価}
処理速度に着目した提案手法の基本的な性能を明らかにするための評価実験について述べる．性能評価の方法と結果および考察について以下に示す．

\vspace{1cm}

\subsection{性能評価の方法}
この実験の目的は，従来手法（Lang-SAM）や第3章の提案手法と比較して処理速度に着目した提案手法がどの程度処理速度を向上させるか，またその際にどの程度の検出位置の正確度を維持できるかを定量的に評価することである．
...
実験結果を\Table{tab:results_improved}に示す．処理速度に着目した提案手法は第3章の提案手法に比べて処理速度が向上していることが確認された．また，従来手法と比較すると，処理速度に着目した提案手法は処理速度を約22倍に向上させることができた（0.79 FPS → 17.39 FPS）．この要因として，第3章の提案手法では比較的小さいモデル（sam2.1\_ hiera\_small）を使用していたものの，セグメンテーション処理を繰り返し実行していたため処理速度が低下していたことが挙げられる．処理速度に着目した提案手法では，これを一定間隔での実行に変更することで処理頻度を低減し，FPSの向上を実現したと考えられる．一方で，BBoxの正確度を示すIoUに関しては，第3章の提案手法と同等の値（0.707）を示した．これは，両手法で同一の物体追跡アルゴリズムを用いているため，正確度を維持できたと考えられる．
...
一方で，セグメンテーションマスクを用いた制御タスクの場合，処理速度に着目した提案手法ではSAM2の高精度なセグメンテーションマスクが毎フレーム得られるわけではなく，一定間隔でしか更新されない．その間，追跡した特徴点群を膨張・複製することでセグメンテーションマスクのように扱うため，処理速度は向上するものの，セグメンテーションマスクの精度は時間経過に伴って劣化する可能性がある．したがって，応答性は向上する一方で，予期せぬ特徴点の喪失や誤追跡などが発生し，制御性能に悪影響を与える可能性があることに留意する必要がある．
